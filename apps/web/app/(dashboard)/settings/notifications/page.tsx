...
const commentNotifications = [
  { id: 'general_comments', title: 'General Comments', description: 'When someone comments on an asset' },
  { id: 'comment_replies', title: 'Comment Replies', description: 'When someone replies to your comment' },
  { id: 'mentions', title: '@Mentions', description: 'When someone @mentions you in a comment' },
]

const assetNotifications = [
  { id: 'other_uploads', title: 'Other Uploads', description: 'When other users upload assets' },
  { id: 'status_updates', title: 'Status Updates', description: "When someone changes an asset's status" },
  { id: 'assigned_to_you', title: 'Assigned to You', description: 'When someone assigns an asset to you' },
]
...
export default function NotificationsPage() {
  ...
  React.useEffect(() => {
    ...
    const notif = (user.preferences.notifications ?? {}) as Partial<NotifPrefs>
    setPrefs({ ...defaults, ...notif })
    setLoaded(true)
  }, [user?.preferences])
    ...
    <div className="p-6 max-w-3xl space-y-8">
      ...
      <section className="space-y-4">
        <div className="flex items-start gap-3 p-4 rounded-lg border border-border bg-bg-secondary">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-medium text-text-primary">Email Notifications Frequency</h3>
            <p className="text-xs text-text-tertiary mt-1">Email updates will be sent to your email address</p>
            <select
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-3">
          {commentNotifications.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-3 rounded-lg border border-border bg-bg-secondary">
              <div>
