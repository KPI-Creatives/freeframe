...
import { Plus, ChevronRight, Filter, X, Trash2, Share2, Copy, Check, Loader2 } from 'lucide-react'
...
import type { Collection, CollectionShare, Project, SharePermission } from '@/types'
...
function CollectionShareDialog({ collection, open, onOpenChange }: CollectionShareDialogProps) {
  ...
  React.useEffect(() => {
    ...
    api
      .get<{ shares: CollectionShare[] }>(`/collections/${collection.id}/shares`)
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const res = await api.post<{ share: CollectionShare }>(`/collections/${collection.id}/share`, body)
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <span className="font-medium text-text-primary capitalize">{share.permission}</span>
                            {share.expires_at && (
...
export default function CollectionsPage() {
  ...
  const [shareCollection, setShareCollection] = React.useState<Collection | null>(null)
